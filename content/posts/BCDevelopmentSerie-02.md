...
Some people think that they can use AI to create the solution for them easily and they do not need the expensive experts anymore. They just need to ask AI to write the code and they will have the solution. But it is not that simple. Even I have heard that it is "recommended way by Microsoft" to do so. If someone tells you "jump from the Window", will you jump? Or you will first check that you are safely tied to the rope? You need to apply same to "use AI".
...
Just today I saw some articles how AI is bad in maintaining code, that most of the time it breaks something else when adding new feature. I know that it will be much better in future, but we are not there yet.
...
- **Do not send customer data to AI** - never paste real customer data, credentials, connection strings or license keys into AI chat. Use anonymized or dummy data when you need to explain data-related problems. And it even means "do not use AI to anonymize customers data".
- **Have a company policy** - your company should have a clear policy about which AI tools are approved, what can and cannot be shared with them, and
how to handle sensitive code. If you do not have one, create it.
- **GitHub with data residency option** - if you are using GitHub, do not forget that there is data residency option. If you use only GitHub.com, data are in US. To be able to choose where the data are stored, you need to buy GitHub with data residency (thus using ghe.com). And it have own challenges to solve.
...
Part of the instructions is even workflow for the agent. For example, if you are using Azure DevOps Work items to describe user stories etc., it is good to describe how the agent should work with the Work Items, when to switch the status, where are acceptance criteria.
...
- **Architecture rules** - when to create new codeunit vs. extend existing one, how to handle events, when to use interfaces, preventing copy-paste, how to structure the solution, use SOLID principles...
- **Error handling** - how to use Error(), Message(), how to handle try functions, what telemetry to add
- **Performance rules** - "always use SetLoadFields", "never use RecordRef when you can use specific record", "always filter before FindSet", use corret ReadIsolation mode, use Queries for data processing, address the fields directly in report dataset etc.
- **Permission sets** - how to structure permissions, what objects to include, when use the inherent permissions
...
```markdown
## AL Code Guidelines
-**General**: Follow event-driven programming model; never modify standard application objects. Use clear, meaningful names and maintain consistent code structure. Prioritize performance optimization and proper error handling. Focus on main application implementation by default. 
- **Performance**: Filter at DB level, use temp tables, Query objects, bulk operations, use SetLoadFields on records and minimize locks by using ReadIsolation explicitely for read operations on records. Filter data early, use temporary tables, avoid unnecessary loops
- **Maintainability**: Clear naming, focused procedures, encapsulate complexity, minimal subscriber code. Give priority for embedding code into descriptive function before adding comments to explain the code
- **SOLID**: Single responsibility, extensibility via events/interfaces, depend on abstractions
- **AL Patterns**: Use events (OnBefore/OnAfter), ErrorInfo, enums over Options, proper permissions
...
### Centralizing the instructions

For GitHub Copilot or Claude you need to have some system how to maintain the instructions centrally for your team. I do not want to have different instructions per each developer or repository. I want everyone to use same instructions for all repositories. There are some standard way how to do that which are using git submodules and symlinks, but I wanted just some simple way. Thus I vibe-coded my own VSCode extension, which is able, based on one centralized setting, to synchronize centrally maintained instructions into any repository, when developer opens the repository (you can gitignore the instruction files if you want, because they will be synced again every time). Check my [Kine Instruction sync](https://marketplace.visualstudio.com/items?itemName=Kine.kine-instruction-sync) extension if you want to use it or get some inspiration for your own solution.
...
- Knowledge about **object numbers, field numbers, enum values** which are already used, tool to **reserve object IDs** from given range
...
I want to share how I use AI in my development workflow today (but it is developing every week). It is not the only way, but it gives you a practical starting point (I am on beginning of this journey, so I am still learning and optimizing my process):

1. **Understand the requirement** - I ask AI to read the Work Item, user story or bug description and ask if everything is understandable, there are no missing things or things which need clarification. If there is something missing, AI gives me the question with some alternatives and I pass these questions back to responsible consultant. This is very important step, because if the requirement is not clear, the code will not be correct.
2. **Let AI analyze and plan** - I ask the AI to analyze the requirement, read existing code and create the implementation plan.
3. **Review the approach** - before any code is written, I review what the AI proposes. Does it make sense? Is it following our architecture? Is it using the right patterns? This is the most important step. If I want some change, I ask AI to make it and write back the changes to Work Item to document the change.
...
- **Provide context** - tell the AI which BC version you are using, which objects are involved, what the current behavior is and what you want to change. "Fix the bug" is bad prompt (it is same like when someone tells you "It doesn't work"). "In codeunit 50100, function PostSalesOrder, the VAT calculation on line 45 is wrong when currency is not LCY - fix it to use the exchange rate from Currency Exchange Rate table" is much better. Or pass the error message including the call stack directly...
...
- In case of **multiple depending apps** you need to handle the splitting of the requirement to separate changes in separate applications. It means it is more complex to use e.g. cloud GitHub Copilot to develop the solution (leading to multiple depending Pull Requests). To solve this, you can use [monorepo](https://en.wikipedia.org/wiki/Monorepo). But again, it have own PROs and CONs.
...
We will talk about CI/CD in more detail in the future parts of this serie, but I want to mention it here because it is so important.
...
Instead of repeating myself, here are the main points:
